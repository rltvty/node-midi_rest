...
output.openVirtualPort("Mixer Control - Output");

module.exports.closePort = output.closePort;
...
function post_info(req, res) {
  if (req.path in settings) {
    ...
    if (current.type in req.body) {
      switch (current.type) {
        case 'level':
          var newValue = parseInt(req.body[current.type]);
      ...
      output.sendMessage(help.cc(current.controller, current.value, current.channel));
